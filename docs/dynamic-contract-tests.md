...
- Response link expressions of the form `$response.body#/...` and `$response.header....` are evaluated at runtime against the response the link is declared on; when the target operation input has a JSON-schema-compatible parameter or request body schema, the resolved value is validated against that target schema. A `$response.header....` expression additionally fails when the response carries more than one header field of that name, because the runtime expression cannot resolve to a single value; links with no evaluable expression are warned with `CONTRACT_LINKS_NOT_VALIDATED`, mixed links with `CONTRACT_LINKS_PARTIALLY_VALIDATED`, and links that omit required target inputs with `CONTRACT_LINK_REQUIRED_INPUT_MISSING`.
