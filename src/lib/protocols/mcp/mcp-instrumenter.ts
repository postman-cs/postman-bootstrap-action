...
const CAPABILITY_BOOLEAN_FIELDS: Record<string, string[]> = {
  tools: ['listChanged'],
  resources: ['subscribe', 'listChanged'],
  prompts: ['listChanged']
};

function validateCapabilities(index: McpContractIndex, warnings: string[]): void {
  const caps = index.capabilities;
  if (!caps) return;
  const shaped: Record<string, JsonRecord | null> = {};
  for (const name of ['tools', 'resources', 'prompts', 'logging', 'completions']) {
    ...
    shaped[name] = record;
    for (const field of CAPABILITY_BOOLEAN_FIELDS[name] ?? []) {
      const flag = record[field];
      if (flag !== undefined && typeof flag !== 'boolean') {
        warnings.push(`MCP_CAPABILITY_FIELD_INVALID: capabilities.${name}.${field} must be a boolean when present; got ${JSON.stringify(flag)}`);
      }
    }
  }

  const declaredFeatures: Array<[string, boolean, string]> = [
    ['tools', index.tools.length > 0, 'tools'],
    ['resources', index.resources.length > 0 || index.resourceTemplates.length > 0, 'resources/resourceTemplates'],
    ['prompts', index.prompts.length > 0, 'prompts']
  ];
  for (const [name, declared, label] of declaredFeatures) {
    if (declared && !shaped[name]) {
      warnings.push(`MCP_CAPABILITY_DECLARED_FEATURE_MISSING: manifest declares ${label} but capabilities.${name} is absent`);
    }
    if (!declared && shaped[name]) {
      warnings.push(`MCP_CAPABILITY_WITHOUT_DECLARATIONS: capabilities.${name} is declared but the manifest has no ${label} declarations`);
    }
  }
}

function validateCommonAnnotations(label: string, annotations: JsonRecord | undefined, warnings: string[]): void {
  if (!annotations) return;
  if (annotations.audience !== undefined) {
    if (!Array.isArray(annotations.audience)) {
      warnings.push(`MCP_ANNOTATIONS_INVALID: ${label} annotations.audience must be an array when present`);
    } else {
      for (const entry of annotations.audience) {
        if (entry !== 'user' && entry !== 'assistant') warnings.push(`MCP_ANNOTATIONS_INVALID: ${label} annotations.audience entries must be user or assistant; got ${JSON.stringify(entry)}`);
      }
    }
  }
  if (annotations.priority !== undefined && (typeof annotations.priority !== 'number' || annotations.priority < 0 || annotations.priority > 1)) {
    warnings.push(`MCP_ANNOTATIONS_INVALID: ${label} annotations.priority must be a number from 0 through 1`);
  }
  if (annotations.lastModified !== undefined && typeof annotations.lastModified !== 'string') {
    warnings.push(`MCP_ANNOTATIONS_INVALID: ${label} annotations.lastModified must be a string when present`);
  }
}

function validateAnnotatedDeclarations(index: McpContractIndex, warnings: string[]): void {
  for (const resource of index.resources) validateCommonAnnotations(`resource ${resource.name}`, resource.annotations, warnings);
  for (const template of index.resourceTemplates) validateCommonAnnotations(`resource template ${template.name}`, template.annotations, warnings);
  for (const prompt of index.prompts) validateCommonAnnotations(`prompt ${prompt.name}`, prompt.annotations, warnings);
}
