...
const PEER_TAGS = {
  ...
  'postman-cs/postman-repo-sync-action': 'v2.10.8',
...
async function evidenceFixture() {
  return buildReleaseEvidenceManifest(baseConfig(), {
    resolveTagCommit: async ({ repository }) => {
      ...
      if (repository === 'postman-cs/postman-bootstrap-action') return RELEASE_SHA;
...
const CORRELATION = 'postman-cs-postman-bootstrap-action-42-1-v9.9.9-deadbeefdeadbeef';
const RUN_TITLE = `release monitor postman-bootstrap-action@${RELEASE_TAG} ${CORRELATION}`;
const EXPECTED = {
  workflow: 'e2e.yml',
  providerTag: PROVIDER_TAG,
  providerCommit: PROVIDER_SHA,
...
test('closed manifest binds the provider, release artifact, and exact six-action closure', async () => {
  ...
  assert.equal(
    evidence.manifest.actions['postman-cs/postman-bootstrap-action'].role,
    'under-test'
  );
  assert.equal(
...
test('complete verifier requires the exact terminal artifact after a successful run', async () => {
  ...
  const title = `release monitor postman-bootstrap-action@${RELEASE_TAG} ${correlationId}`;
...
test('terminal artifact lookup rejects an incomplete artifact census', async () => {
  ...
  const exactRun = run({
    display_title: `release monitor postman-bootstrap-action@${RELEASE_TAG} ${correlationId}`
  });
