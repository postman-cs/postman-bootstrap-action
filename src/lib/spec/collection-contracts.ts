...
export function createContractScript(operation: ContractOperation, warnings: string[] = []): string[] {
  ...
  return [
    ...
    '      var linkHeaderMatches = 0;',
    '      if (typeof pm.response.headers.each === "function") pm.response.headers.each(function (headerField) { if (headerField && headerField.key && String(headerField.key).toLowerCase() === String(expression.header).toLowerCase()) linkHeaderMatches += 1; });',
