...
| `CONTRACT_BODYLESS_STATUS_WITH_CONTENT` | The spec declares response content for a HEAD operation or a 1xx/204/205/304 response, which RFC 9110 forbids on the wire. | Remove the content declaration or use a body-capable method/status. |
| `CONTRACT_RESPONSE_BODY_UNDOCUMENTED` | A body-capable response omits `content` or uses `content: {}`. Body presence, media type, and shape are unknown rather than an empty-body contract. | Add response media types and schemas before using body checks as strict CI gates. |
| `CONTRACT_RESPONSE_SCHEMA_UNDOCUMENTED` | A response media entry declares no `schema`. Body presence and media type are checked, but body shape is not. | Add a schema to the response media entry. |
| `CONTRACT_REQUEST_SCHEMA_UNDOCUMENTED` | A request-body media entry declares no `schema`, so the generated request payload shape is not validated. | Add a schema to the request-body media entry. |
...
| `CONTRACT_ROOT_SCRIPT_LARGE` | The consolidated contract root script passed the byte advisory threshold. | Non-fatal. Split the spec or reduce per-operation schema size if the collection nears the hard limit. |
| `CONTRACT_ROOT_SCRIPT_PROLOGUE_INVALID` | An operation's generated prologue did not declare its contract binding, so the shared runtime could not be bound. | Report as a bug: the contract index produced an operation the script builder cannot bind. |
| `CONTRACT_ROOT_SCRIPT_SHARDED` | The consolidated contract runtime did not fit one root script and was split across multiple collection-root test events. | Non-fatal. Each shard is self-contained; reduce spec size if shard count grows. |
| `CONTRACT_ROOT_SCRIPT_UNPARSABLE` | A generated consolidated root script failed to parse, which would silence every contract assertion. | Report as a bug: generation fails closed rather than persisting an unparsable script. |
| `CONTRACT_ROOT_SCRIPT_SIZE_EXCEEDED` | A single operation's contract runtime cannot fit the hard root-script byte limit even in its own shard. | Reduce that operation's schema size; the generator fails closed rather than truncating assertions. |
...
| `CONTRACT_COLLECTION_LINK_NOT_STABLE` | Generated collections did not remain linked to the canonical specification for two consecutive readbacks. | Retry after Spec Hub propagation recovers. |
