...
If you do not set those inputs, the action preserves today’s bootstrap behavior.
...
| `collection-sync-mode` | `reuse` | Collection lifecycle policy. `reuse` keeps existing collections, `refresh` regenerates the current collection set from the latest spec, and `version` creates or reuses release-scoped collections. |
...
- `reuse`: current behavior. Existing collection IDs are reused when available.
...
Create a side-by-side versioned spec and collection set without moving the current pointers:
