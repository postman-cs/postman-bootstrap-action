...
- Validate concrete request parameter values at runtime: scalar query/header/path/cookie parameters, decodable array parameters (exploded `form` arrays from repeated query keys; non-exploded `form`, `spaceDelimited`, and `pipeDelimited` arrays split on their delimiter; comma-joined `simple` header arrays), required-cookie presence, and concrete JSON request bodies against the request-side OpenAPI schemas; placeholder values such as `<string>` and `{{variables}}` are skipped.
